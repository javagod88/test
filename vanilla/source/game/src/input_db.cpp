...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	if (!SECTREE_MANAGER::instance().GetValidLocation(pTab->lMapIndex, pTab->x, pTab->y, lMapIndex, pos, d->GetEmpire()))
	{
		...
		sys_log(0, "%s moved to villiage of %u, map_index %d", 
							pTab->name, d->GetEmpire(), g_start_map[d->GetEmpire()]);

		char szQuery[QUERY_MAX_LEN];

		snprintf(szQuery, sizeof(szQuery), "UPDATE player SET map_index=%ld , x=%ul , y=%ul WHERE `name`='%s'", g_start_map[d->GetEmpire()], g_start_position[d->GetEmpire()][0], g_start_position[d->GetEmpire()][1], pTab->name);
