...
void CHARACTER::SetItem(TItemPos Cell, LPITEM pItem)
{
	...
	if (pItem)
	{
		...
		case INVENTORY:
		case EQUIPMENT:
			if ((wCell < inventorysize) || (BELT_INVENTORY_SLOT_START <= wCell && BELT_INVENTORY_SLOT_END > wCell))
...
LPITEM CHARACTER::GetWear(int bCell) const
{
	int inventorysize = INVENTORY_MAX_NUM;
	if(!bigger_inventory) inventorysize = 90;
	...
	return m_pointsInstant.pItems[inventorysize + bCell];
}

void CHARACTER::SetWear(int bCell, LPITEM item)
{
	// > WEAR_MAX_NUM : 용혼석 슬롯들.
	if (bCell >= WEAR_MAX_NUM + DRAGON_SOUL_DECK_MAX_NUM * DS_SLOT_MAX)
	{
	...
	int inventorysize = INVENTORY_MAX_NUM;
	if(!bigger_inventory) inventorysize = 90;
	SetItem(TItemPos (INVENTORY, inventorysize + bCell), item);
	...
	case INVENTORY:
		{
			...
			if (Cell.IsBeltInventoryPosition())
			{
				...
				if (NULL == beltItem)
				{
					if(belt_force_slots <= bCell - BELT_INVENTORY_SLOT_START)
						return false;
				} else {
			...
			if (m_pointsInstant.bItemGrid[bCell])
			{
				if (m_pointsInstant.bItemGrid[bCell] == iExceptionCell)
				{
					...
					int j = 1;
					BYTE bPage;
					if(bigger_inventory == true)
					{
						bPage = bCell / (inventorysize / 4);
					}
					else {
						bPage = bCell / (inventorysize / 2);
					}
					do
					{
						BYTE p = bCell + (5 * j);
						...
						if(bigger_inventory == true)
						{
							if (p / (inventorysize / 4) != bPage)
								return false;
						} else {
							if (p / (inventorysize / 2) != bPage)
								return false;
						}
			...
			else
			{
				...
				do
				{
					...
					if (bigger_inventory == true && (p / (inventorysize / 4) != bPage))
						return false;
					if (bigger_inventory == false && (p / (inventorysize / 2) != bPage))
						return false;
...
bool CHARACTER::RefineInformation(int bCell, BYTE bType, int iAdditionalCell)
{
	int inventorysize = INVENTORY_MAX_NUM;
	if(!bigger_inventory) inventorysize = 90;
	if (bCell > inventorysize)
		return false;
							...
							case 70206:   // 염색약(검은색)
								{
									...
									else
									{
										...
										if (pPC)
										{
											...
											if (last_dye_level == 0 ||
													last_dye_level+dye_level <= GetLevel() ||
													item->GetVnum() == 70201)
											{
							...
							switch (item->GetSubType())
							{
								...
								case USE_ADD_ATTRIBUTE :
									...
									if (item2->GetAttributeCount() < 4)
									{
										...
										} else if (attr_always_5_add == true) {
											aiItemAttributeAddPercent[4] = 100;
										}
								...
								case USE_ADD_ATTRIBUTE2 :
									...
									if (item2->GetAttributeCount() == 4)
									{
										...
										if ((attr_always_5_add == true) || (number(1, 100) <= aiItemAttributeAddPercent[4]))
										{
											item2->AddAttribute();
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("속성 추가에 성공하였습니다."));
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You have to wait until you can drop items again."));
		return false;
	}
...
bool CHARACTER::DropGold(int gold)
{
	...
	if (item)
	{
		...
		if (item->AddToGround(GetMapIndex(), pos))
		{
			...
			item->StartDestroyEvent(gold_drop_time);
			if(gold_drop_time>=60)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("떨어진 아이템은 %d분 후 사라집니다."), gold_drop_time/60);
			else
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	int inventorysize = INVENTORY_MAX_NUM;
	if(!bigger_inventory) inventorysize = 90;
	if (INVENTORY == Cell.window_type && Cell.cell >= inventorysize && IS_SET(item->GetFlag(), ITEM_FLAG_IRREMOVABLE))
		return false;
	...
	if (DestCell.IsEquipPosition())
	{
		...
		int inventorysize = INVENTORY_MAX_NUM;
		if(!bigger_inventory) inventorysize = 90;
		EquipItem(item, DestCell.cell - inventorysize);
	}
	else
	{
...
bool CHARACTER::SwapItem(int bCell, int bDestCell)
{
	...
	if (TItemPos(EQUIPMENT, item2->GetCell()).IsEquipPosition())
	{
		int inventorysize = INVENTORY_MAX_NUM;
		if(!bigger_inventory) inventorysize = 90;
		int bEquipCell = item2->GetCell() - inventorysize;
		int bInvenCell = item1->GetCell();
	...
	{
		int bCell1 = item1->GetCell();
		int bCell2 = item2->GetCell();
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (item->IsDragonSoul())
	{
		...
		// 용혼석은 swap을 지원하면 안됨.
		if(GetInventoryItem(inventorysize + iWearCell))
		{
	...
	else
	{
		...
		if (GetWear(iWearCell) && !IS_SET(GetWear(iWearCell)->GetFlag(), ITEM_FLAG_IRREMOVABLE))
		{
			...
			if (false == SwapItem(item->GetCell(), inventorysize + iWearCell))
			{
				return false;
			}
		}
