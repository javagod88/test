...
void CBuffOnAttributes::RemoveBuffFromItem(LPITEM pItem)
{
	...
	if (NULL != pItem)
	{
		if (pItem->GetCell() < inventorysize)
			return;
		std::vector <BYTE>::iterator it = find (m_p_vec_buff_wear_targets->begin(), m_p_vec_buff_wear_targets->end(), pItem->GetCell() - inventorysize);
