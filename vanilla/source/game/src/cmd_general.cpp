...
ACMD(do_execute_quest)
{
	//return;
	//char arg1[256];
	//one_argument(argument, arg1, sizeof(arg1));
	//quest::PC* pPC = quest::CQuestManager::instance().GetCurrentPC();
	//quest::QuestState * pOldState = pPC->GetRunningQuestState();
	//int iNowState = pOldState->st;
	//pOldState = 0;
	//DWORD dwQuestIndex = quest::CQuestManager::instance().GetQuestIndexByName(argument);
	quest::CQuestManager::instance().Execute(ch->GetPlayerID());
}
...
ACMD(do_view_equip)
{
	if(!ch)
		return;
	if ((view_equip_for_all == false) && (ch->GetGMLevel() <= GM_PLAYER))
		return;
	...
	if (*arg1)
	{
		...
		if (!tch->IsPC())
			return;
		if(view_equip_sp_cost > 0)
		{
			int iSPCost = (ch->GetMaxSP() * 100) / view_equip_sp_cost;
...
ACMD(do_inventory)
{
	...
	{
		str_to_number(index, arg1); index = MIN(index, inventorysize);
		str_to_number(count, arg2); count = MIN(count, inventorysize);
	}
...
ACMD(do_elevate)
{
	if((ch->GetElevation() == true) || (ch->GetRealGMLevel() == GM_PLAYER))
		return;
	char a1[256];
