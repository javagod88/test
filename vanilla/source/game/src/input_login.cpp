...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (g_bCheckClientVersion)
	{
		...
		sys_log(0, "VERSION CHECK %d %d %s %s", version, date, gServerVersion.c_str(), d->GetClientVersion());
	...
	if(ch->GetQuestFlag((const char*)"antiexp.active")==1)
		ch->block_exp = true;
