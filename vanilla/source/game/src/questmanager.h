...
namespace quest
{
	...
	class CQuestManager : public singleton<CQuestManager>
	{
		...
		public:
			...
			void		Player_Kill(unsigned int pc, unsigned int npc);
			void		UniqueKill(unsigned int pc, unsigned int npc);
			void		Kill(unsigned int pc, unsigned int npc);
			void		Buy(unsigned int pc);
			...
			void		Execute(DWORD pc);
			void		Message(unsigned int pc, unsigned int npc);
