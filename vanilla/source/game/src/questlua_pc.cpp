...
namespace quest 
{
	...
	int pc_send_effect(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
		{
			sys_err("pc_send_effect: No character!!!");
			return 0;
		}
		if (!lua_isnumber(L, 1)) 
		{
		...
		ch->EffectPacket((int)lua_tonumber(L, 1));
		return 0;
	}
	...
	int pc_delayed_disconnect(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
		{
		...
		if (!lua_isnumber(L, 1)) 
		{
			sys_log(0, "pc_delayed_disconnect: No time. Setting to 10.");
		} else {
			time = (int)lua_tonumber(L, 1);
		}
		ch->GetDesc()->DelayedDisconnect(time);
		return 1;
	}
	...
	int pc_get_version(lua_State* L)
	{
		...
		if(!ch)
		{
			sys_err("pc_get_version: No character!!!");
			return 0;
		}
	...
	int pc_set_level(lua_State * L)  
	{
		...
		else
		{
			...
			ch->PointChange(POINT_STAT, ((MINMAX(1, newLevel, gPlayerMaxLevelStats) - ch->GetLevel()) * 3) + ch->GetPoint(POINT_LEVEL_STEP));
			//레벨
	...
	int pc_change_money(lua_State * L)
	{
		...
		if ((long long int)gold + ch->GetGold() < 0)
	...
	int pc_have_map_scroll(lua_State* L)
	{
		...
		for (int iCell = 0; iCell < inventorysize; iCell++)
		{
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushboolean(L, ch->GetRealGMLevel() >= GM_HIGH_WIZARD);
		return 1;
	}
	...
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch->GetRealGMLevel());
		return 1;
	}
...
teleport_area:
	...
	int pc_get_socket_items( lua_State* L )
	{
		...
		// 용혼석 슬롯은 할 필요 없을 듯.
		// 이 함수는 탈석서용 함수인 듯 하다.
		for ( int i=0; i < inventorysize + WEAR_MAX_NUM; i++ )
		{
	...
	int pc_block_exp(lua_State* L)
	{
		...
		if(!ch)
			return 1;
		ch->block_exp = true;
		ch->SetQuestFlag((const char*)"antiexp.active", 1);
		return 0;
	}
		
	int pc_unblock_exp(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
			return 1;
		ch->block_exp = false;
		ch->SetQuestFlag((const char*)"antiexp.active", 0);
		return 0;
	}

	int pc_tmp_var_set(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
			return 1;
		if(!lua_isstring(L,1) || !lua_isstring(L,2))
		{
			sys_err("QUEST: pc_tmp_var_set expects two strings!!!");
			return 1;
		}
		std::string key = std::string(lua_tostring(L,1));
		if(key.size() == 0)
		{
			sys_err("QUEST: pc_tmp_var_set key is empty!!!");
			return 1;
		}
		std::string val = std::string(lua_tostring(L,2));
		ch->SetTmpVar(key, val);
		return 0;
	}
	int pc_tmp_var_get(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
			return 1;
		if(!lua_isstring(L,1))
		{
			sys_err("QUEST: pc_tmp_var_get no key!!!");
			return 1;
		}
		std::string key = std::string(lua_tostring(L,1));
		ch->DelTmpVar(key);
	...
	int pc_tmp_var_del(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if(!ch)
			return 1;
		if(!lua_isstring(L,1))
		{
			sys_err("QUEST: pc_tmp_var_del no key!!!");
			return 1;
		}
		std::string key = std::string(lua_tostring(L,1));
		std::string result = ch->GetTmpVar(key);
	...
	//int pc_get_id(lua_State *L)
	//{
		//return pc_get_player_id(L);
		//LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		//if( ch != NULL )
		//{
		//	lua_pushnumber(L, ch->GetPlayerID() );
	...
	void RegisterPCFunctionTable()
	{
		luaL_Reg pc_functions[] = 
		{
			...
			{ "get_id",	pc_get_player_id	},
			{ "send_effect", pc_send_effect },
			{ "delayed_disconnect", pc_delayed_disconnect },
			{ "get_mount_vnum", pc_get_mount_vnum },
			{ "get_version", pc_get_version },
			{ "temp_var_set", pc_tmp_var_set },
			{ "temp_var_get", pc_tmp_var_get },
			{ "temp_var_delete", pc_tmp_var_del },
