...
namespace quest
{
	...
	int dungeon_new_jump(lua_State* L)
	{
		...
		long lMapIndex = (long)lua_tonumber(L,1);
		long x = (long)lua_tonumber(L, 2) * 100;
		long y = (long)lua_tonumber(L, 3) * 100;
		...
		ch->WarpSet(x, y, pDungeon->GetMapIndex());
		return 0;
	}
	...
	int dungeon_kill_unique(lua_State* L)
	{
		...
		if (pDungeon)
			pDungeon->KillUnique(lua_tostring(L,1), pChar);
