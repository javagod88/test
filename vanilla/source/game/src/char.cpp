...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
	...
	if (dwPlayedTime > 60000)
	{
		if (GetSectree() && !GetSectree()->IsAttr(GetX(), GetY(), ATTR_BANPK))
		{
			...
			{
				int initial = rank_points_floor_initial_c;
				if (IsEquipUniqueItem(UNIQUE_ITEM_FASTER_ALIGNMENT_UP_BY_TIME))
					UpdateAlignment((initial * 2) * (dwPlayedTime / 60000));
				else
					UpdateAlignment(initial * (dwPlayedTime / 60000));
			}
			else
			{
				int initial = rank_points_floor_initial;
				UpdateAlignment(initial * (dwPlayedTime / 60000));
			}
		}
...
void CHARACTER::SetLevel(long level)
{
...
void CHARACTER::ComputePoints()
{
	...
	if (IsPC())
	{
		...
		SetPoint(POINT_MOV_SPEED,	movement_speed);
...
int CHARACTER::GetPoint(BYTE type) const
{
	...
	if (val > max_val) {
		sys_err("POINT_ERROR: %s type %d val %d (max: %d) RESET TO max_val", GetName(), val, max_val);
		val = max_val;
	}
...
void CHARACTER::PointChange(BYTE type, int amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			sys_log(0, "LEVELUP: %s %d NEXT EXP %ulld", GetName(), GetLevel(), GetNextExp());

			// WOLFMAN 수인족 특수처리 (수인족은 직군이 하나이므로, 5레벨이 되면 무조건 1번 직군으로 설정함. 하드코딩 ㅈㅅ)
			if (race_5_enable == true && GetJob() == JOB_WOLFMAN)
			{
				if (5 <= val)
					SetSkillGroup(1);
				else
			...
			if (amount)
			{
				...
				LogManager::instance().LevelLog(this, val2, GetRealPoint(POINT_PLAYTIME) + (get_dword_time() - m_dwPlayStartTime) / 60000);
		...
		case POINT_EXP:
			{
				...
				else
				{
					...
					long long int q = (long long int)(next_exp / 4.0f);
		...
		case POINT_GOLD:
			{
				...
				if (nTotalMoney > yang_max)
				{
					sys_err("[OVERFLOW_GOLD] maximum %lld OriGold %lld AddedGold %d id %u Name %s ", yang_max, GetGold(), amount, GetPlayerID(), GetName());
		...
		case POINT_MELEE_MAGIC_ATT_BONUS_PER:
			...
			{
				amount = 100;
				sys_err("MALL_BONUS corrected to 100! point type: %d name: %s amount %d", type, GetName(), amount);
			} else {
		...
		case POINT_POTION_BONUS:	// 74
			...
			{
				amount = 100;
				sys_err("BONUS corrected to 100! point type: %d name: %s amount %d", type, GetName(), amount);
			} else {
				amount = (GetPoint(type) + amount);
			}
...
{
	if(elevation_enable && GetElevation() == false)
		return GM_PLAYER;
	if (test_server)
...
void CHARACTER::SetGodSpeed(bool mode)
{
	if (m_pkMobInst != NULL)
	{
		...
		if (mode == true)
		{
			SetPoint(POINT_ATT_SPEED, mob_max_attack_speed);
		}
		else
		{
			SetPoint(POINT_ATT_SPEED, m_pkMobData->m_table.sAttackSpeed);
...
unsigned long long int CHARACTER::GetNextExp() const
{
	if (PLAYER_EXP_TABLE_MAX < GetLevel())
		return exp_need_threshold;
	else
...
{
    for (unsigned int i = 0; i < SpamBlockListArray.size(); i++)
    {
...
void CHARACTER::SetTmpVar(std::string key, std::string var)
{
	int c = 0;
	for(auto const& i : m_vTmpVar)
	{
		if(key.compare(i.first) == 0)
		{
			m_vTmpVar.erase(m_vTmpVar.begin() + c);
		}
	}
	m_vTmpVar.push_back(std::pair<std::string, std::string>(key, var));
}

void CHARACTER::DelTmpVar(std::string key)
{
	int c = 0;
	for(auto const& i : m_vTmpVar)
	{
...
std::string CHARACTER::GetTmpVar(std::string key)
{
	std::string result = "";
	for(auto const& i : m_vTmpVar)
	{
