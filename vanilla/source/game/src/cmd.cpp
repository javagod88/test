...
//	{ "phase_tbipbv-igb-iyfi-prii-su-",	do_phase,		0,			POS_DEAD,	GM_PLAYER	},
	...
	{ "execute_quest", do_execute_quest, 0, POS_DEAD, GM_PLAYER	},
...
void interpret_command(LPCHARACTER ch, const char * argument, size_t len)
{
	...
	if(ch->GetLastInterruptTime() >= (now + interrupt_interval))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can not execute commands that fast!"));
		return;
	}
	ch->SetLastInterruptTime(now);
