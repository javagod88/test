...
ACMD(do_warp)
{
	...
	else
	{
		...
		else
		{
			if(tch->GetMapIndex() == ch->GetMapIndex())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "You warp to the target.");
				ch->Show(ch->GetMapIndex(), tch->GetX(), tch->GetY(), 0);
				ch->Stop();
				return;
			}
			x = tch->GetX() / 100;
		...
		case 0:	// gold
			{
				...
				long long int before_gold = tch->GetGold();
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'b':
				...
				LoadBlockSpamList();
				LoadBannSpamList();
				sys_log(0, "Reloading ban/spam list infomations.");
				break;
		}
	}
	else
		...
		case 'c':
			{
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					if (ch->GetGold() < (long long int)t->dwPrice)
					{
...
ACMD(do_horse_level)
{
	...
	str_to_number(level, arg2);
	level = MINMAX(0, level, max_horse_level);
...
ACMD(do_set_socket)
{
	...
	int item_id, socket_num, value = 0;
