...
namespace quest
{
	...
	void CQuestManager::unique_init()
	{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("player_kill", QUEST_PLAYER_KILL_EVENT));		// Mob을 사냥
		m_mapEventName.insert(TEventNameMap::value_type("player_party_kill", QUEST_PLAYER_PARTY_KILL_EVENT));		// Mob을 사냥
		m_mapEventName.insert(TEventNameMap::value_type("kill", QUEST_KILL_EVENT));		// Mob을 사냥
		m_mapEventName.insert(TEventNameMap::value_type("unique_kill", QUEST_UNIQUE_KILL_EVENT));		// Mob을 사냥
		...
		m_mapEventName.insert(TEventNameMap::value_type("boot", QUEST_BOOT_EVENT));	// Boot
		...
		m_mapEventName.insert(TEventNameMap::value_type("buy", QUEST_BUY_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("die", QUEST_DEAD_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("execute", QUEST_EXECUTE_EVENT));
		m_mapEventName.insert(TEventNameMap::value_type("message", QUEST_PLAYER_MESSAGE_EVENT));
	...
	void CQuestManager::Buy(unsigned int pc)
	{
		PC * pPC;
			if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
				return;
				m_mapNPC[QUEST_NO_NPC].OnBuy(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}
	...
	void CQuestManager::Message(unsigned int pc, unsigned int pc2)
	{
		//m_CurrentNPCRace = npc;
		PC * pPC;

		sys_log(0, "CQuestManager::Message QUEST_MESSAGE_EVENT (pc=%d, pc=%d)", pc, pc2);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/   
			// call script
			bool called = false;
			if(m_mapNPC[pc2].OnMessage(*pPC))
				called = true;

			if(called == false)
				if (m_mapNPC[QUEST_NO_NPC].OnMessage(*pPC))
					return;
		}
		else
			sys_err("QUEST: no such pc id : %d", pc);
	...
	void CQuestManager::Execute(unsigned int pc)
	{
		PC* pPC;
		sys_log(0, "CQuestManager::Execute QUEST_EXECUTE_EVENT (pc=%d)", pc);
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
				return;

			m_mapNPC[QUEST_NO_NPC].OnExecute(*pPC);
		}
		else
			sys_err("CQuestManager::Execute QUEST_EXECUTE_EVENT no such pc id : %d", pc);
	}

	void CQuestManager::Player_Kill(unsigned int pc, unsigned int pc2)
	{
		//m_CurrentNPCRace = npc;
		PC * pPC;

		sys_log(0, "CQuestManager::Player_Kill QUEST_PLAYER_KILL_EVENT (pc=%d, pc=%d)", pc, pc2);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/   
			// call script
			bool called = false;
			if(m_mapNPC[pc2].OnPlayerKill(*pPC))
				called = true;
			...
			if (leader)
			{
				...
				if (m_mapNPC[pc2].OnPartyKill(*GetPC(leader->GetPlayerID())))
					return;
			...
			if(called == false)
				if (m_mapNPC[QUEST_NO_NPC].OnPlayerKill(*pPC))
					return;
		}
		else
			sys_err("QUEST: no such pc id : %d", pc);
	...
	void CQuestManager::UniqueKill(unsigned int pc, unsigned int npc)
	{
		//m_CurrentNPCRace = npc;
		PC * pPC;

		sys_log(0, "CQuestManager::UniqueKill QUEST_KILL_EVENT (pc=%d, npc=%d)", pc, npc);
		...
		if ((pPC = GetPC(pc)))
		{
			...
			*/   
			...
			if (npc != QUEST_NO_NPC)
				m_mapNPC[QUEST_NO_NPC].OnUniqueKill(*pPC);
		}
		else
			sys_err("QUEST: no such pc id : %d", pc);
	}
	...
	{
		sys_log(0, "XXX On Boot %p", GetPCForce(0));
		m_pCurrentPC = GetPCForce(0);
		m_pCurrentCharacter = NULL;
		m_pSelectedDungeon = NULL;
	...
	void CQuestManager::Reload(LPCHARACTER ch)
	{
		if (!InitializeLua())
		{
			fprintf(stderr, "Unable to reload quest\n");
			ch->ChatPacket(CHAT_TYPE_INFO, "Unable to reload quest.");
			return;
		}
		...
		{
			fprintf(stderr, "Unable to reload quest\n");
			ch->ChatPacket(CHAT_TYPE_INFO, "Quest crashed!!!");
			return;
		}
