		...
		if (GetLevel() - pkAttacker->GetLevel() >= -10)
		{
			if (pkAttacker->GetRealAlignment() < 0)
			{
				if (pkAttacker->IsEquipUniqueItem(UNIQUE_ITEM_FASTER_ALIGNMENT_UP_BY_KILL))
					pkAttacker->UpdateAlignment(rank_points_from_monsters_c * 2);
				else
					pkAttacker->UpdateAlignment(rank_points_from_monsters_c);
			}
			else
				pkAttacker->UpdateAlignment(rank_points_from_monsters);
		}

		pkAttacker->SetQuestNPCID(GetVID());
		quest::CQuestManager::instance().Kill(pkAttacker->GetPlayerID(), GetRaceNum());
		if(IsPC())
			quest::CQuestManager::instance().Player_Kill(pkAttacker->GetPlayerID(), GetPlayerID());
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if (IsPC())
		{
			...
			if(IsUnique())
			{
				quest::CQuestManager::instance().UniqueKill(pkKiller->GetPlayerID(), quest::QUEST_NO_NPC);
				GetUnique()->RemoveUnique(std::to_string(GetVID()));
				SetUnique(nullptr);
			}
			quest::CQuestManager::instance().Player_Kill(pkKiller->GetPlayerID(), GetPlayerID());
		...
		else
		{
			...
			else
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked)
				{
					...
					{
						long int penalty = (rank_points_penalty * (-10));
						if (g_iUseLocale && pkKiller->GetParty())
						{
							long int penalty_grp = (rank_points_group_penalty * (-10));
							FPartyAlignmentCompute f(penalty_grp, pkKiller->GetX(), pkKiller->GetY());
							pkKiller->GetParty()->ForEachOnlineMember(f);

							if (f.m_iCount == 0)
								pkKiller->UpdateAlignment(penalty);
							else
							{
	...
	if (true == IsMonster() && 2493 == GetMobTable().dwVnum)
	{
		if(pkKiller)
		{
			if (NULL != pkKiller && NULL != pkKiller->GetGuild() && pkKiller->GetMapIndex() == 408)
			{
				CDragonLairManager::instance().OnDragonDead( this, pkKiller->GetGuild()->GetID() );
			}
			else if (NULL != pkKiller)
			{
				//std::string asdf = ;
				sys_log(0, "DragonLair: Dragon killed by %s", pkKiller->GetName());
				//CDragonLairManager::instance().OnDragonDead( this, 0 );
			}
		} else {
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	// 게임내 기본 제공되는 경험치 보너스
	{
		...
		if (to->GetPoint(POINT_EXP_DOUBLE_BONUS))
			iExp += (iExp * to->GetPoint(POINT_EXP_DOUBLE_BONUS) / 100); // NEW FUNC EXP NO_BUG
//			if (number(1, 100) <= to->GetPoint(POINT_EXP_DOUBLE_BONUS))
//				iExp += iExp * to->GetPoint(POINT_EXP_DOUBLE_BONUS) / 100; // 1.3배 (30%)
