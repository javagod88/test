...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	switch (pinfo->sub_header)
	{
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
				const long long int nTotalGold = ch->GetExchange()->GetCompany()->GetOwner()->GetGold() + pinfo->arg1;

				if (nTotalGold > yang_max)
				{
...
//	if (!test_server)	//2012.05.15 김용욱 : 테섭에서 (무적상태로) 다수 몬스터 상대로 다운되면서 공격시 콤보핵으로 죽는 문제가 있었다.
	{
		...
		if (((false == ch->IsRiding() && fDist > 50) || fDist > 80) && OXEVENT_MAP_INDEX != ch->GetMapIndex())
		{
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	if(bigger_inventory == false) inventorysize = 90;
