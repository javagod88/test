...
public:
	...
	static bool IsAvailableCell(WORD cell, int beltGrade /*int beltLevel*/)
	{
		...
		if (cell <= belt_force_slots)
			result = true;

		if(ruleTable[cell] <= beltGrade)
			result = true;
	...
	static bool IsItemInSlotBeltInventory(LPCHARACTER pc, DWORD vnum, int target_cell)
	{
		...
		if(!vnum)
		{
			pc->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ERROR! No vnum specified!"));
			return false;
		}
		if(!target_cell)
		{
			pc->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ERROR! No target cell specified!"));
			return false;
		}
		LPITEM beltInventoryItem = pc->GetInventoryItem(target_cell);
