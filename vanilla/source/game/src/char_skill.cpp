...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	DWORD nextTime = get_global_time() + sb_delay;
	...
	if (pkSk->dwType != 0)
	{
		...
		switch (GetSkillMasterType(pkSk->dwVnum))
		{
			case SKILL_NORMAL:
				...
				if (GetSkillLevel(pkSk->dwVnum) >= skill_master_upgrade)
				{
					if (GetQuestFlag("reset_scroll.force_to_master_skill") > 0 || skill_force_master)
					{
...
bool CHARACTER::CheckSkillHitCount(const BYTE SkillID, const VID TargetVID)
{
	...
	std::unordered_map<VID, size_t, std::hash<DWORD>>::iterator iterTargetMap = rSkillUseInfo.TargetVIDMap.find(TargetVID);
