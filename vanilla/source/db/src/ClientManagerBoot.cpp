...
//			isNameFile = false;
		...
		query << "SELECT vnum, name, " << g_stLocaleNameColumn << ", type, rank, battle_type, level, size+0, ai_flag+0, setRaceFlag+0, setImmuneFlag+0, on_click, empire, drop_item, resurrection_vnum, folder, st, dx, ht, iq, damage_min, damage_max, max_hp, regen_cycle, regen_percent, exp, gold_min, gold_max, def, attack_speed, move_speed, aggressive_hp_pct, aggressive_sight, attack_range, polymorph_item, enchant_curse, enchant_slow, enchant_poison, enchant_stun, enchant_critical, enchant_penetrate, resist_sword, resist_twohand, resist_dagger, resist_bell, resist_fan, resist_bow, resist_fire, resist_elect, resist_magic, resist_wind, resist_poison, dam_multiply, summon, drain_sp, skill_vnum0, skill_level0, skill_vnum1, skill_level1, skill_vnum2, skill_level2, skill_vnum3, skill_level3, skill_vnum4, skill_level4 , sp_berserk, sp_stoneskin, sp_godspeed, sp_deathblow, sp_revive  FROM mob_proto ORDER BY vnum"; 
		SQLMsg* sqlMsg = CDBManager::instance().DirectQuery(query.str().c_str(), 0); 
		if (sqlMsg == nullptr) { 
			sys_err("libdb::InitializeMobTable", __LINE__, "Could not load mob_proto. Query failed!"); 
			return false; 
		} 
		MYSQL_RES* pRes = sqlMsg->Get()->pSQLResult; 

		if (!pRes) { 
			sys_err("libdb::InitializeMobTable", __LINE__, "Could not load mob_proto. No result!"); 
			return false; 
		} 

		if (!m_vec_mobTable.empty()) { 
			sys_log(__LINE__, " Reloading mob_proto"); 
			m_vec_mobTable.clear(); 
		} 
		MYSQL_ROW row; 
		while ((row = mysql_fetch_row(pRes))) { 
			SMobTable temp = {}; 
			if (row[0]) temp.dwVnum = strtoul(row[0], 0, 10); 
			if (row[1]) snprintf(temp.szName, 25, "%s", row[1]); 
			if (row[2]) snprintf(temp.szLocaleName, 25, "%s", row[2]); 
			if (row[3]) temp.bType = strtoul(row[3], 0, 10); 
			if (row[4]) temp.bRank = strtoul(row[4], 0, 10); 
			if (row[5]) temp.bBattleType = strtoul(row[5], 0, 10); 
			if (row[6]) temp.bLevel = strtoul(row[6], 0, 10); 
			if (row[7]) temp.bSize = strtoul(row[7], 0, 10); 
			if (row[8]) temp.dwAIFlag = strtoul(row[8], 0, 10); 
			if (row[9]) temp.dwRaceFlag = strtoul(row[9], 0, 10); 
			if (row[10]) temp.dwImmuneFlag = strtoul(row[10], 0, 10); 
			if (row[11]) temp.bOnClickType = strtoul(row[11], 0, 10); 
			if (row[12]) temp.bEmpire = strtoul(row[12], 0, 10); 
			if (row[13]) temp.dwDropItemVnum = strtoul(row[13], 0, 10); 
			if (row[14]) temp.dwResurrectionVnum = strtoul(row[14], 0, 10); 
			if (row[15]) snprintf(temp.szFolder, 65, "%s", row[15]); 
			if (row[16]) temp.bStr = strtoul(row[16], 0, 10); 
			if (row[17]) temp.bDex = strtoul(row[17], 0, 10); 
			if (row[18]) temp.bCon = strtoul(row[18], 0, 10); 
			if (row[19]) temp.bInt = strtoul(row[19], 0, 10); 
			if (row[20]) temp.dwDamageRange[0] = strtoul(row[20], 0, 10); 
			if (row[21]) temp.dwDamageRange[1] = strtoul(row[21], 0, 10); 
			if (row[22]) temp.dwMaxHP = strtoul(row[22], 0, 10); 
			if (row[23]) temp.bRegenCycle = strtoul(row[23], 0, 10); 
			if (row[24]) temp.bRegenPercent = strtoul(row[24], 0, 10); 
			if (row[25]) temp.dwExp = strtoul(row[25], 0, 10); 
			if (row[26]) temp.dwGoldMin = strtoul(row[26], 0, 10); 
			if (row[27]) temp.dwGoldMax = strtoul(row[27], 0, 10); 
			if (row[28]) temp.wDef = strtol(row[28], 0, 10); 
			if (row[29]) temp.sAttackSpeed = strtol(row[29], 0, 10); 
			if (row[30]) temp.sMovingSpeed = strtol(row[30], 0, 10); 
			if (row[31]) temp.bAggresiveHPPct = strtol(row[31], 0, 10); 
			if (row[32]) temp.wAggressiveSight = strtol(row[32], 0, 10); 
			if (row[33]) temp.wAttackRange = strtol(row[33], 0, 10); 
			if (row[34]) temp.dwPolymorphItemVnum = strtoul(row[34], 0, 10); 
			if (row[35]) temp.cEnchants[0] = strtol(row[35], 0, 10); 
			if (row[36]) temp.cEnchants[1] = strtol(row[36], 0, 10); 
			if (row[37]) temp.cEnchants[2] = strtol(row[37], 0, 10); 
			if (row[38]) temp.cEnchants[3] = strtol(row[38], 0, 10); 
			if (row[39]) temp.cEnchants[4] = strtol(row[39], 0, 10); 
			if (row[40]) temp.cEnchants[5] = strtol(row[40], 0, 10); 
			if (row[41]) temp.cResists[0] = strtol(row[41], 0, 10); 
			if (row[42]) temp.cResists[1] = strtol(row[42], 0, 10); 
			if (row[43]) temp.cResists[2] = strtol(row[43], 0, 10); 
			if (row[44]) temp.cResists[3] = strtol(row[44], 0, 10); 
			if (row[45]) temp.cResists[4] = strtol(row[45], 0, 10); 
			if (row[46]) temp.cResists[5] = strtol(row[46], 0, 10); 
			if (row[47]) temp.cResists[6] = strtol(row[47], 0, 10); 
			if (row[48]) temp.cResists[7] = strtol(row[48], 0, 10); 
			if (row[49]) temp.cResists[8] = strtol(row[49], 0, 10); 
			if (row[50]) temp.cResists[9] = strtol(row[50], 0, 10); 
			if (row[51]) temp.cResists[10] = strtol(row[51], 0, 10); 
			if (row[52]) temp.fDamMultiply = strtod(row[52], 0); 
			if (row[53]) temp.dwSummonVnum = strtoul(row[53], 0, 10); 
			if (row[54]) temp.dwDrainSP = strtoul(row[54], 0, 10); 
			if (row[55]) temp.Skills[0].dwVnum = strtoul(row[55], 0, 10); 
			if (row[56]) temp.Skills[0].bLevel = strtoul(row[56], 0, 10); 
			if (row[57]) temp.Skills[1].dwVnum = strtoul(row[57], 0, 10); 
			if (row[58]) temp.Skills[1].bLevel = strtoul(row[58], 0, 10); 
			if (row[59]) temp.Skills[2].dwVnum = strtoul(row[59], 0, 10); 
			if (row[60]) temp.Skills[2].bLevel = strtoul(row[60], 0, 10); 
			if (row[61]) temp.Skills[3].dwVnum = strtoul(row[61], 0, 10); 
			if (row[62]) temp.Skills[3].bLevel = strtoul(row[62], 0, 10); 
			if (row[63]) temp.Skills[4].dwVnum = strtoul(row[63], 0, 10); 
			if (row[64]) temp.Skills[4].bLevel = strtoul(row[64], 0, 10); 
			if (row[65]) temp.bBerserkPoint = strtoul(row[65], 0, 10); 
			if (row[66]) temp.bStoneSkinPoint = strtoul(row[66], 0, 10); 
			if (row[67]) temp.bGodSpeedPoint = strtoul(row[67], 0, 10); 
			if (row[68]) temp.bDeathBlowPoint = strtoul(row[68], 0, 10); 
			if (row[69]) temp.bRevivePoint = strtoul(row[69], 0, 10); 
...
void CClientManager::CheckItemTable()
{
	std::ostringstream query; 
	query << "SHOW COLUMNS FROM item_proto WHERE field='vnum_range'"; 
	SQLMsg* sqlMsg = CDBManager::instance().DirectQuery(query.str().c_str(), 0); 
	if (sqlMsg == nullptr) { 
		sys_err("libdb::CheckItemTable", __LINE__, "Could not load item_proto. Query failed!"); 
		return; 
	} 
	...
	{
		sys_err("libdb::CheckItemTable: Found a mismatch! Attempting to upgrade your item_proto...");
		std::ostringstream query2; 
		query2 << "ALTER TABLE item_proto ADD vnum_range tinyint(4) NOT NULL DEFAULT 0"; 
		SQLMsg* sqlMsg2 = CDBManager::instance().DirectQuery(query2.str().c_str(), 0); 
		if (sqlMsg2 == nullptr) { 
			sys_err("libdb::CheckItemTable", __LINE__, "Could not upgrade. Query failed!"); 
			return; 
		}
		sys_err("libdb::CheckItemTable: Upgrading item_proto success! Ready to go!");
		everything_ok = true;
		delete(sqlMsg2);
		sqlMsg2 = nullptr;
	}
	...
	if(everything_ok == true)
		sys_log(0, "libdb::CheckItemTable: Sanity check for vnum_range is OK!");
	else
		sys_log(0, "libdb::CheckItemTable: Sanity check for vnum_range is not okay!!!!");
	mysql_free_result(pRes);
	delete(sqlMsg);
	sqlMsg = nullptr;
}
...
//			isNameFile = false;
		...
		if(!test_data.Load("item_proto_test.txt",'\t'))
		{
			...
			//2. 테스트 아이템 테이블 생성.
			TItemTable * test_item_table = nullptr;
...
//		int testValue =  m_vec_itemTable.size();
		...
		while ((row = mysql_fetch_row(pRes))) { 
			SItemTable temp = {}; 
			if (row[0]&&*row[0]) temp.dwVnum = strtoul(row[0], 0, 10); 
			if (row[1]&&*row[1]) temp.bType = strtoul(row[1], 0, 10); 
			if (row[2]&&*row[2]) temp.bSubType = strtoul(row[2], 0, 10); 
			if (row[3]&&*row[3]) snprintf(temp.szName, 25, "%s", row[3]); 
			if (row[4]&&*row[4]) snprintf(temp.szLocaleName, 25, "%s", row[4]); 
			if (row[5]&&*row[5]) temp.dwGold = strtoul(row[5], 0, 10); 
			if (row[6]&&*row[6]) temp.dwShopBuyPrice = strtoul(row[6], 0, 10); 
			if (row[7]&&*row[7]) temp.bWeight = strtoul(row[7], 0, 10); 
			if (row[8]&&*row[8]) temp.bSize = strtoul(row[8], 0, 10); 
			if (row[9]&&*row[9]) temp.dwFlags = strtoul(row[9], 0, 10); 
			if (row[10]&&*row[10]) temp.dwWearFlags = strtoul(row[10], 0, 10); 
			if (row[11]&&*row[11]) temp.dwAntiFlags = strtoul(row[11], 0, 10); 
			if (row[12]&&*row[12]) temp.dwImmuneFlag = strtoul(row[12], 0, 10); 
			if (row[13]&&*row[13]) temp.dwRefinedVnum = strtoul(row[13], 0, 10); 
			if (row[14]&&*row[14]) temp.wRefineSet = strtoul(row[14], 0, 10); 
			if (row[15]&&*row[15]) temp.bAlterToMagicItemPct = strtoul(row[15], 0, 10); 
			if (row[16]&&*row[16]) temp.bGainSocketPct = strtoul(row[16], 0, 10); 
			if (row[17]&&*row[17]) temp.sAddonType = strtoul(row[17], 0, 10); 
			if (row[18]&&*row[18]) temp.aLimits[0].bType = strtoul(row[18], 0, 10); 
			if (row[19]&&*row[19]) temp.aLimits[0].lValue = strtoul(row[19], 0, 10); 
			if (row[20]&&*row[20]) temp.aLimits[1].bType = strtoul(row[20], 0, 10); 
			if (row[21]&&*row[21]) temp.aLimits[1].lValue = strtoul(row[21], 0, 10); 
			if (row[22]&&*row[22]) temp.aApplies[0].bType = strtoul(row[22], 0, 10); 
			if (row[23]&&*row[23]) temp.aApplies[0].lValue = strtoul(row[23], 0, 10); 
			if (row[24]&&*row[24]) temp.aApplies[1].bType = strtoul(row[24], 0, 10); 
			if (row[25]&&*row[25]) temp.aApplies[1].lValue = strtoul(row[25], 0, 10); 
			if (row[26]&&*row[26]) temp.aApplies[2].bType = strtoul(row[26], 0, 10); 
			if (row[27]&&*row[27]) temp.aApplies[2].lValue = strtoul(row[27], 0, 10); 
			if (row[28]&&*row[28]) temp.alValues[0] = strtoul(row[28], 0, 10); 
			if (row[29]&&*row[29]) temp.alValues[1] = strtoul(row[29], 0, 10); 
			if (row[30]&&*row[30]) temp.alValues[2] = strtoul(row[30], 0, 10); 
			if (row[31]&&*row[31]) temp.alValues[3] = strtoul(row[31], 0, 10); 
			if (row[32]&&*row[32]) temp.alValues[4] = strtoul(row[32], 0, 10); 
			if (row[33]&&*row[33]) temp.alValues[5] = strtoul(row[33], 0, 10); 
			if (row[34]&&*row[34]) temp.dwVnumRange = strtoul(row[34], 0, 10); 
