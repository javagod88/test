...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				"SELECT "
				"id,account_id,name,job,voice,dir,x,y,z,map_index,exit_x,exit_y,exit_map_index,hp,mp,stamina,random_hp,random_sp,playtime,"
...
bool CreatePlayerTableFromRes(MYSQL_RES * res, TPlayerTable * pkTab)
{
	...
	str_to_number(pkTab->account_id, row[col++]);
		...
		case QID_QUEST:
			{
				...
				if( pLoginData1 == NULL )
					break;
				if( strlen(pLoginData1->GetAccountRef().login) == 0)
					break;
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
	...
	queryLen = snprintf(queryStr, sizeof(queryStr), 
			...
			"VALUES(0, %u, '%s', %ld, %d, %d, %d, %d, "
			"%d, %d, %d, %d, %d, %d, %ld, "
			"%d, %ld, %d, %d, %d, %d, %d, 0, %lld, 0, ",
			GetTablePostfix(),
	...
	sys_log(0, "PlayerCreate accountid %d name %s level %ld gold %lld, st %d ht %d job %d",
			packet->account_id, 
